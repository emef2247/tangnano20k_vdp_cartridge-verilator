    ...
    with path.open('r', encoding='utf-8', errors='replace') as f:
        for raw in f:
            ...
            if is_match and not printed_header:
                ...
                print(f'ADDRESS,{format_hex(address,4)}')
                printed_header = True
            ...
            if parsed['type'] == 'change':
                ...
                if is_match:
                    ...
                    print(f'INFO,"frame={parsed["frame"]} time={parsed["time"]} reg={reg_text} val={val_text}"')
                    if delta > 0:
                        print(f'CYCLE,{step_cycles_count}')
                    printed_any = True
            ...
            if is_match:
                print(f'INFO,"frame={parsed["frame"]} time={parsed["time"]} port={port_text} value={value_text}"')
                if delta > 0:
                    print(f'CYCLE,{step_cycles_count}')
                printed_any = True
            ...
            if port in ADDR_PORTS:
                if control_low is None:
                    ...
                    if is_match:
                        print(f'ADDRESS,{format_hex(new_addr,4)},{format_hex(high,2)},{format_hex(control_low,2)}')
                    address = new_addr
                    control_low = None
                continue
            ...
            if port in DATA_PORTS:
                if is_match:
                    print(f'IO,address,{format_hex(value,2)},{format_hex(value,2)}')
                    # print('address++;')
